...
class Step2 extends React.Component {
	...
	sendsms(phone) {
		...
		console.log('1', phone);
		Axios.post(`${API}/sms/Verfy`, { phone: phone })
			.then((response) => {
	...
	verifyCode(value, phone) {
		...
		Axios.post(`${API}/sms/CheckVerfy`, { phone: phone, code: value })
			.then((response) => {
				//	this.setState({ dates: response.data });
						...
						<GridContainer>
							{this.state.verfied ? (
								<Button
									...
									size="lg"
									block
									onClick={(e) => this.createUser(this.props.allStates.Registeration)}
								>
									Create User{' '}
								</Button>
							) : this.state.sentVerfication ? (
								<CustomInput
									...
									inputProps={{
										onChange     : (event) =>
											this.handleChange(event, 'code', this.props.allStates.Registeration.phone),
										endAdornment : (
								...
								<Button
									...
									size="lg"
									block
									onClick={(e) => this.sendsms(this.props.allStates.Registeration.phone)}
								>
