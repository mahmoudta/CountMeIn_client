			...
			response.data.map((appointment) => {
				...
				this.setState((prevState) => ({
					tableContent : [
						...
						[
							appointment[2],
							<Link to={`/business/view/${appointment[0]}`}>{appointment[3]}</Link>,
							appointment[4],
				...
				<GridContainer>
					<GridItem xs={12}>
						<Card>
							...
							<CardBody>
								<Table
									tableHead={[ '#', 'Business', 'Start Time', 'Date', 'Service', 'Actions' ]}
...
export default connect(mapStateToProps)(
	withStyles(extendedTablesStyle, sweetAlertStyle, userProfileStyles, dashboardStyle)(Upcomming)
);
