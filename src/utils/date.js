...
export function getTime(date) {
  if (date) {
    ...
    return {
      ...
      minute:
        Number(Ndate.getUTCMinutes()) < 10
          ? `0${Ndate.getUTCMinutes()}:`
          : Ndate.getUTCMinutes()
    };
  }
}
...
export function getBusinessTime(working, date) {
  ...
  if (!isEmpty(working) && day > -1) {
    if (working[day].opened) {
      ...
      let diff = Math.ceil(
        Number(getTimeDifference(working[day].from, working[day].until))
      );
      var times = [];
...
export function getAppointmentTime(start, end) {
  ...
  const start_minute =
    Number(start._minute) < 10 ? `0${start._minute}` : start._minute;
